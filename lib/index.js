...
function checkAppPath (appPath) {
  ...
  const coreVersion = pkg.dependencies['@lianjia-fe/bucky-core']
  const needCoreVersion = require('../package.json').coreVersion

  if ( !coreVersion ) return noBucky()
  if (coreVersion !== needCoreVersion) {
    console.log('\n  ' +
      `New version of bucky-core available ${coreVersion} -> ${needCoreVersion}\n  ` +
      `Please upgrade bucky-core's version in "packge.json" file and reinstall`
    )
  }
  return true
  ...
  function noBucky () {
    console.log('\n  not found application base on bucky-core')
  }
}
